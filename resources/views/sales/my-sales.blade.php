<!--Vista de listado de ventas que tiene el usuario identificado.
    Muestra una lista con todas las ventas del usuario.
    Filtra las ventas.
    Pagina las ventas.
-->
<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 ">
            ...
            <nav class="navbar navbar-light py-6 mb-4"> 
               <div class="bg-white py-4 px-2 rounded border-2 border-gray-300">
                    ...
                    <form class="form-inline pt-4" method="GET" action="{{ route('my.sales',['pagination'=>4])}}" >

                        <!--Lista todas piezas que dispone el usuario:-->
            ...
            <div class="hidden sm:flex mt-8 mb-1  ">
                 <x-dropdown width="48">
                     ...
                     <x-slot name="content">
                         <form method="GET">
                             <x-dropdown-link :href="route('my.sales',['pagination'=>4])">{{ __('Paginación de 4') }}</x-dropdown-link>
                             <x-dropdown-link :href="route('my.sales',['pagination'=>6])">{{ __('Paginación de 6') }}</x-dropdown-link>
                             <x-dropdown-link :href="route('my.sales',['pagination'=>8])">{{ __('Paginación de 8') }}</x-dropdown-link>
                             <x-dropdown-link :href="route('my.sales',['pagination'=>10])">{{ __('Paginación de 10') }}</x-dropdown-link>
                         </form>
                     </x-slot>
                 </x-dropdown>
            </div>
                ...
                <table class="overflow-x-auto overflow-y-auto w-full bg-white divide-y divide-gray-200 mt-4">
                    <thead class="bg-blue-300">
                        <tr class="divide-x">
                            ...
                            <th class="px-3 py-2 text-xs font-medium text-gray-700 font-bold uppercase">Acción</th>
                        </tr>
                    </thead>
                    ...
                    @if($success)
                        @foreach($sales as $sale)     
                            <tr class="text-center">
                                ...
                                <td class="py-3">
                                    <x-nav-link  :href="route('edit.sale', ['id'=> $sale->id])" class="font-bold" >
                    ...
                    @else    
                        <tr class="text-center">
                            <td colspan="4" class="py-3 font-bold text-red-600 text-lg">{{ $message }}</td>
                        </tr>
